...
function Invoke-DirectoryCreate {
  ...
  process {
    try {
      if (Test-Path -Path $component.Path -PathType 'Container' -ErrorAction 'SilentlyContinue') {
        Write-Log -verbose:$verbose -logName $eventLogName -source $eventLogSource -severity 'info' -message ('{0} ({1}) :: detected directory {2}.' -f  $($MyInvocation.MyCommand.Name), $component.ComponentName, $component.Path)
      } else {
        New-Item -Path $component.Path -ItemType 'directory' -force | Out-Null
        if (Test-Path -Path $component.Path -PathType 'Container' -ErrorAction 'SilentlyContinue') {
          Write-Log -verbose:$verbose -logName $eventLogName -source $eventLogSource -severity 'info' -message ('{0} ({1}) :: created directory {2}.' -f  $($MyInvocation.MyCommand.Name), $component.ComponentName, $component.Path)
        } else {
          Write-Log -verbose:$verbose -logName $eventLogName -source $eventLogSource -severity 'error' -message ('{0} ({1}) :: failed to create directory {2}.' -f  $($MyInvocation.MyCommand.Name), $component.ComponentName, $component.Path)
        }
      }
    } catch {
      Write-Log -verbose:$verbose -logName $eventLogName -source $eventLogSource -severity 'error' -message ('{0} ({1}) :: error creating directory {2}. {3}' -f  $($MyInvocation.MyCommand.Name), $component.ComponentName, $component.Path, $_.Exception.Message)
    }
  }
  end {
  ...
  process {
    ...
    try {
      ...
      while ($timer.Elapsed.TotalSeconds -lt $waitInSeconds) {
        Write-Log -verbose:$verbose -logName $eventLogName -source $eventLogSource -severity 'info' -message ('{0} ({1}) :: command ({2} {3}) has a wait of {4} seconds configured. {5} seconds elapsed so far.' -f $($MyInvocation.MyCommand.Name), $componentName, $command, ($argumentList -join ' '), $waitInSeconds, $timer.Elapsed.TotalSeconds)
